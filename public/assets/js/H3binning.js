...
function bufferPointsLinear(geojson, radius, h3Resolution) {
  if (geojson == null)
    return;
  //console.log('bufferPointsLinear fn()')
  const layer = {};
...
async function getData() {
  ...
  //06 May 2023. We dont needs the point weight - we need the hexagonal bining - which changes the size of the hexagons
  //Thus, we can change this - buts lete suse the same variable to represent hexagons
  ...
  //console.log('\thexagonal_binning_resolution '+ hexagonal_binning_resolution + '\t bufferRadiusValue ' + buffer_radius); 
  //console.log('\tpoints_cloud_weight_more '+ points_cloud_weight_more + '\t bufferRadiusValue_more ' + buffer_radius_more); 

  //lets check for this value first, else exit
  //if (map_data_loaded == false) // bufferRadiusValue == 0) //for when the page loads the first time, as there is no dataset loaded yet, buffer radious value is 0 in our case
...
async function getData_more_binned_layer() {
  ...
  var hexagonal_binning_resolution_more = hexagonal_binning_resolution - 1;     //we reduce the resolution by one  ..before it was divide by 2 which was inccoreect way of doing it
...
function renderHexes(map, hexagons, hexagons_more_binned) {
  //lets check for this value first, else exit
//  if (hexagons == null) //for when the page loads the first time, as there is no dataset loaded yet
//    return;                     //just exit to prevent a null error later for 'hexagons' variable 
