...
//var decrypted_zip_file = null;  //hold the decrypted zip file which will be later used to display the dataset
var decrypted_buf;
...
var decryptedGEOJSON;   //hold the decryptedGEOJSON which will be later used to display the dataset
...
//level 3 = coarse, level 2 = medium, level 1 = fine. Fine indside medium which would be inside coarse
                                //objFile, levelToDecryptTo, currentLevel - 1, download, startTimeDEC
async function multiLevelDecrypt(v_objFile, levelToDecryptTo, currentLevel, downloadFile, startTimeDEC){ // when this function is called at first the value for `currentLevel' should be 1, then when called recursively it increases
 ...
 // 28 May 2023 This code should be shifted  HBbinning_verification.js      
 //The next 33 lines - marked with end should be commented/taken out and shifted              
                    zipfile = zip; 
                    file_name = filename;
                    // 23-May-2023 We have shifted this block of code to the H3Binning_verification.js file
...
//                        console.log("3 levels of decryption complete. Exceution Time: " + executionTimeDEC);
                        (async () => {
                            ...
                            const buf = await blob.arrayBuffer(); //blobToArrayBuffer(blob); //await blob.arrayBuffer();
 ...
 //end                       
            ...
            */  //if binning was performed
                ...
                else if (filename.endsWith('.enc') == true && currentLevel != levelToDecryptTo) //havent reaced the required level, we keep on decrypting
                {
                    ...
                    zip.file(filename).async('blob').then( (blob) => {                            
                        //call the recursive decryption //objFile, levelToDecryptTo, currentLevel - 1, download, startTimeDEC
